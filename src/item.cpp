...
void Item::put_in(Item payload) { contents.push_back(payload); }
...
std::string Item::save_info()
{
    ...
    if (is_gun() && curammo != NULL)
        ammotmp = curammo->id;
    dump << " " << int(invlet) << " " << int(type->id) << " " << int(charges) << " " << int(damage)
         << " " << int(ammotmp) << " " << int(bday);
    if (active)
        dump << " 1";
    else
        dump << " 0";
...
void Item::load_info(std::string data, Game* g)
{
    ...
    dump >> lettmp >> idtmp >> charges >> damtmp >> ammotmp >> bday >> acttmp >> owntmp;
    if (idtmp == itm_corpse) {
        int corp;
        dump >> corp;
...
std::string Item::info(bool showtext)
{
    ...
    dump << " Volume: " << volume() << "    Weight: " << weight() << "\n"
         << " Bash: " << int(type->melee_dam) << "  Cut: " << int(type->melee_cut)
         << "  To-hit bonus: " << (type->m_to_hit > 0 ? "+" : "") << int(type->m_to_hit) << "\n";
    ...
    if (is_food()) {
        ...
        dump << " Skill used: " << skill_name(gun->skill_used) << "\n Ammunition: " << clip_size()
             << " rounds of " << ammo_name(ammo())
             << "\n Damage bonus: " << (gun_damage() > 0 ? "+" : "") << gun_damage()
        ...
        if (mod->accuracy != 0)
            dump << " Accuracy: " << (mod->accuracy > 0 ? "+" : "") << int(mod->accuracy);
        if (mod->damage != 0)
        ...
        if (book->type == sk_null)
            ...
            dump << " Can bring your " << skill_name(book->type) << " skill to " << int(book->level)
                 << std::endl;
            if (book->req == 0)
                dump << " It can be understood by beginners.\n";
            else
                dump << " Requires " << skill_name(book->type) << " level " << int(book->req)
                     << " to understand.\n";
        }
        dump << " Requires intelligence of " << int(book->intel) << std::endl;
        if (book->fun != 0)
            dump << " Reading this book affects your morale by " << (book->fun > 0 ? "+" : "")
                 << int(book->fun) << std::endl;
...
char Item::symbol() { return type->sym; }
...
std::string Item::tname()
{
    ...
    if (damage != 0) {
        ...
        case IRON:
            if (damage == 1)
                damtext = "lightly rusted ";
            if (damage == 2)
                damtext = "rusted ";
            if (damage == 3)
                damtext = "very rusty ";
            if (damage == 4)
                damtext = "thoroughly rusted ";
            break;
        default:
            damtext = "damaged ";
        }
...
unsigned int Item::weight()
{
    ...
    for (auto& content : contents) {
        ret += static_cast<int>(content.weight());
    }
...
unsigned char Item::volume_contained()
{
    ...
    for (auto& content : contents) {
        ret += static_cast<int>(content.volume());
    }
...
int Item::weapon_value(int skills[num_skill_types])
{
    ...
    my_value += int(type->melee_dam * (1 + .3 * skills[sk_bashing] + .1 * skills[sk_melee]));
    // debugmsg("My value: (+bash) %d", my_value);

    my_value += int(type->melee_cut * (1 + .4 * skills[sk_cutting] + .1 * skills[sk_melee]));
...
bool Item::is_two_handed(player* u)
{
    ...
    return volume() > 10 || static_cast<int>(weight()) > u->str_cur * 4;
}
...
bool Item::is_gunmod() { return type->is_gunmod(); }

bool Item::is_ammo() { return type->is_ammo(); }
...
bool Item::is_food(player* u)
{
    ...
    if (u->has_bionic(bio_batteries) && is_ammo()
        ...
        return true;
    if (u->has_bionic(bio_furnace) && is_flammable(type->m1) && is_flammable(type->m2))
        return true;
    return false;
}

bool Item::is_food_container(player* u) { return (contents.size() >= 1 && contents[0].is_food(u)); }
...
bool Item::is_food_container() { return (contents.size() >= 1 && contents[0].is_food()); }

bool Item::is_drink() { return type->is_food() && type->m1 == LIQUID; }

bool Item::is_weap()
{
    if (is_gun() || is_food() || is_ammo() || is_food_container() || is_armor() || is_book()
        || is_tool())
        return false;
    return (type->melee_dam > 7 || type->melee_cut > 5);
}

bool Item::is_bashing_weapon() { return (type->melee_dam > 7); }

bool Item::is_cutting_weapon() { return (type->melee_cut > 5); }

bool Item::is_armor() { return type->is_armor(); }

bool Item::is_book() { return type->is_book(); }

bool Item::is_container() { return type->is_container(); }

bool Item::is_tool() { return type->is_tool(); }
...
int Item::clip_size()
{
    ...
    for (auto& content : contents) {
        if (content.is_gunmod()) {
            int bonus = (ret * (dynamic_cast<it_gunmod*>(content.type))->clip) / 100;
            ret += bonus;
        }
    }
...
int Item::accuracy()
{
    ...
    for (auto& content : contents) {
        if (content.is_gunmod()) {
            ret -= dynamic_cast<it_gunmod*>(content.type)->accuracy;
        }
    }
...
int Item::gun_damage()
{
    ...
    for (auto& content : contents) {
        if (content.is_gunmod()) {
            ret += dynamic_cast<it_gunmod*>(content.type)->damage;
        }
    }
...
int Item::noise()
{
    ...
    for (auto& content : contents) {
        if (content.is_gunmod()) {
            ret += dynamic_cast<it_gunmod*>(content.type)->loudness;
        }
    }
...
int Item::burst_size()
{
    ...
    for (auto& content : contents) {
        if (content.is_gunmod()) {
            ret += dynamic_cast<it_gunmod*>(content.type)->burst;
        }
    }
...
int Item::recoil()
{
    ...
    for (auto& content : contents) {
        if (content.is_gunmod()) {
            ret += dynamic_cast<it_gunmod*>(content.type)->recoil;
        }
    }
...
ammotype Item::ammo()
{
    if (is_gun()) {
        ...
        for (auto& content : contents) {
            if (content.is_gunmod()) {
                const auto* mod {dynamic_cast<const it_gunmod*>(content.type)};

                if (mod->newtype != ammotype::AT_NULL) {
                    ret = mod->newtype;
                }
            }
        }
        ...
        if (charges > 0) {
            ...
            for (std::size_t i {0}; i < u.inv.size(); ++i) {
                if (u.inv[i].type->id == aid) {
                    am.push_back(static_cast<int>(i));
                }
            }
        } else {
            am = u.has_ammo(ammo());
...
Choose ammo type:         Damage     Armor Pierce     Range     Accuracy");
            ...
            for (std::size_t i {0}; i < am.size(); ++i) {
                ...
                mvwprintw(w_ammo, static_cast<int>(i) + 1, 3, "%s (%d)",
                          u.inv[am[i]].tname().c_str(), u.inv[am[i]].charges);
                mvwprintw(w_ammo, static_cast<int>(i) + 1, 27, "%d", ammo_type->damage);
                mvwprintw(w_ammo, static_cast<int>(i) + 1, 38, "%d", ammo_type->pierce);
                mvwprintw(w_ammo, static_cast<int>(i) + 1, 55, "%d", ammo_type->range);
                mvwprintw(w_ammo, static_cast<int>(i) + 1, 65, "%d", 100 - ammo_type->accuracy);
            }
            ...
            do {
                ch = getch();
            } while ((ch < 'a' || ch - 'a' > static_cast<int>(am.size()) - 1) && ch != ' '
                     && ch != 27);
            ...
            for (const int current_index : am) {
                if (u.inv[current_index].type->id == curammo->id
                    && u.inv[current_index].charges < smallest) {
                    smallest = u.inv[current_index].charges;
                    index = current_index;
                }
            }
        }
...
bool is_flammable(material m)
{
    if (m == VEGGY || m == FLESH || m == POWDER || m == COTTON || m == WOOL || m == LEATHER
        || m == PAPER || m == WOOD || m == MNULL)
        return true;
    return false;
}
} // namespace oocdda
