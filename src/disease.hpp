...
void dis_effect(game* g, player& p, disease& dis)
{
    ...
    case DI_SLEEP:
        ...
        if (rng(2, 80) + rng(0, 100) + rng(0, 120) + rng(0, p.fatigue) < g->light_level()
            && (one_in(p.fatigue / 10) || p.fatigue < 10)) {
    ...
    case DI_DRUNK:
        ...
        if (dis.duration <= 600)
            p.str_cur += 1;
        if (dis.duration > 2000 + 100 * dice(2, 100) && (p.has_trait(PF_WEAKSTOMACH) || one_in(20)))
            p.vomit(g);
...
Strength - 1;     Dexterity - 4;";
    case DI_DRUNK:
        ...
        stream << "Strength - " << strpen << ";    Dexterity - " << allpen << ";    Intelligence - "
               << allpen << ";    Perception - " << allpen;
