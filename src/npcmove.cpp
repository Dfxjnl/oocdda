...
void npc::move(game* g)
{
    ...
    if (want_to_attack_player(g))
        ...
        action = npc_reload;
    else if (attitude == NPCATT_HEAL && g->m.sees(posx, posy, g->u.posx, g->u.posy, light, linet)) {
...
npc_action npc::method_of_attacking_player(game* g, std::vector<point>& path)
{
    ...
    if (can_see_u)
        path = line_to(posx, posy, g->u.posx, g->u.posy, linet);
    else if (saw_player_recently() && g->m.sees(posx, posy, plx, ply, g->light_level(), linet))
        path = line_to(posx, posy, plx, ply, linet);
    ...
    if (can_see_u) {
        ...
        if (weapon.is_gun()) {
            if (weapon.charges > 0) {
                ...
                else if (can_reload() && dist - confident_range() >= weapon.reload_time(*this))
...
bool npc::fetching_item() { return (itx != 999 && ity != 999); }

bool npc::saw_player_recently() { return (plx != 999 && ply != 999); }

bool npc::has_destination() { return (goalx != 999 && goaly != 999); }

bool npc::alt_attack_available()
{
    if (has_amount(itm_grenade, 1) || has_amount(itm_grenade_act, 1) || has_amount(itm_molotov, 1)
        || has_amount(itm_molotov_lit, 1) || has_amount(itm_gasbomb, 1)
        || has_amount(itm_gasbomb_act, 1) || has_amount(itm_dynamite, 1)
        || has_amount(itm_dynamite_act, 1) || has_amount(itm_mininuke, 1)
        || has_amount(itm_mininuke_act, 1))
        return true;
    return false;
}
...
npc_action npc::long_term_goal_action(game* g, std::vector<point>& path)
{
    ...
    if (attitude == NPCATT_FOLLOW || attitude == NPCATT_FOLLOW_RUN || attitude == NPCATT_SLAVE) {
        if (g->sees_u(posx, posy, linet)) {
            path = line_to(posx, posy, g->u.posx, g->u.posy, linet);
            return npc_follow_player;
        } else if (saw_player_recently() && g->m.sees(posx, posy, plx, ply, light, linet)) {
            path = line_to(posx, posy, plx, ply, linet);
            return npc_follow_player;
        } else
            return npc_look_for_player;
...
void npc::move_to(game* g, int x, int y)
{
    ...
    if (abs(x - posx) > 1 || abs(y - posy) > 1) {
        debugmsg("Tried to move_to more than one space! (%d, %d) to (%d, %d)", posx, posy, x, y);
        return;
    }
...
void npc::alt_attack(game* g, monster* m, player* p)
{
    ...
    if (throwing) {
        ...
        case itm_grenade:
            ...
            activated->active = true;
            if (seen)
                g->add_msg("%s pulls the pin on %s grenade.", name.c_str(), (male ? "his" : "her"));
            break;
        }
    }
}
...
void npc::heal_player(game* g, player& patient)
{
    ...
    if (!patient.is_npc()) {
        ...
        if (op_of_u.value * 4 + op_of_u.trust + personality.altruism * 3
                ...
                + (fac_has_job(FACJOB_DOCTORS) ? 15 : 0) - op_of_u.fear * 3
            < 25) {
