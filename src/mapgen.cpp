                   ...
                   int turn)
{
        ...
        for (int i = 0; i < SEEX * 2; i++) {
            for (int j = 0; j < SEEX * 2; j++) {
                if ((j == 4 || j == 8) && (i == 6 || i == 10 || i == 14 || i == 18))
                    ter(i, j) = t_gas_pump;
                else if ((j < 2 && i > 7 && i < 16) || (j < 12 && i > 1 && i < SEEX * 2 - 2))
                    ter(i, j) = t_pavement;
                    ...
                    ter(i, j) = t_rack;
                else if ((i == 14 && j > 12 && j < 16) || (j == 15 && i > 13 && i < 18))
                    ter(i, j) = t_counter;
                else if (i > 4 && i < SEEX * 2 - 5 && j > 12 && j < SEEY * 2 - 3)
                    ter(i, j) = t_floor;
                else
    ...
    case ot_s_sports_west:
        ...
        for (int i = 0; i < SEEX * 2; i++) {
            for (int j = 0; j < SEEY * 2; j++) {
                if (((j == tw || j == bw) && i >= lw && i <= rw) || (j == cw && i > lw && i < rw))
                    ter(i, j) = t_wall_h;
        ...
        for (int i = 0; i < SEEX * 2; i++) {
            for (int j = 0; j < SEEY * 2; j++) {
                if ((i == 5 || i == SEEX * 2 - 6) && j > 7 && j < 19)
                    ...
                    ter(i, j) = t_counter;
                else if (i < 18 && ((j == 8 && i > 5) || (j == 10 && i > 7) || (j == 12 && i > 9)))
                    ter(i, j) = t_rack;
    ...
    case ot_set_bionics:
        for (int i = 0; i < SEEX * 2; i++) {
            for (int j = 0; j < SEEY * 2; j++) {
                if ((j == 8 && i > 1 && i < SEEX * 2 - 2) || (j == 13 && i > 2 && i < SEEX * 2 - 3))
                    ter(i, j) = t_wall_h;
                else if (j == 19) {
                    if (i < 2 || i > SEEX * 2 - 3)
                        ter(i, j) = grass_or_dirt();
    ...
    case ot_rock:
        if (t_north == ot_cavern || t_north == ot_slimepit || t_north == ot_slimepit_down)
            n_fac = 6;
        else
            n_fac = 0;
        if (t_east == ot_cavern || t_east == ot_slimepit || t_east == ot_slimepit_down)
            e_fac = 6;
        else
            e_fac = 0;
        if (t_south == ot_cavern || t_south == ot_slimepit || t_south == ot_slimepit_down)
            s_fac = 6;
        else
            s_fac = 0;
        if (t_west == ot_cavern || t_west == ot_slimepit || t_west == ot_slimepit_down)
            w_fac = 6;
        else
            w_fac = 0;
        ...
        for (int i = 0; i < SEEX * 2; i++) {
            for (int j = 0; j < SEEY * 2; j++) {
                if (rng(0, n_fac) > j || rng(0, e_fac) > SEEX * 2 - 1 - i || rng(0, w_fac) > i
                    || rng(0, s_fac) > SEEY * 2 - 1 - j)
                    ter(i, j) = t_rock_floor;
                else
                    ter(i, j) = t_rock;
            }
    ...
    case ot_slimepit_down:
        ...
        if (t_above == ot_slimepit_down) {
            switch (rng(1, 4)) {
            case 1: {
                ter(rng(0, 2), rng(0, 2)) = ter_id::t_slope_up;
            } break;

            case 2: {
                ter(rng(0, 2), SEEY * 2 - rng(1, 3)) = ter_id::t_slope_up;
            } break;

            case 3: {
                ter(SEEX * 2 - rng(1, 3), rng(0, 2)) = ter_id::t_slope_up;
            } break;

            case 4: {
                ter(SEEX * 2 - rng(1, 3), SEEY * 2 - rng(1, 3)) = ter_id::t_slope_up;
            } break;
            }
        }
    ...
    case ot_subway_station:
        if (t_north >= ot_subway_ns && t_north <= ot_subway_nesw && connects_to(t_north, 2))
            n_fac = 1;
        if (t_east >= ot_subway_ns && t_east <= ot_subway_nesw && connects_to(t_east, 3))
            e_fac = 1;
        if (t_south >= ot_subway_ns && t_south <= ot_subway_nesw && connects_to(t_south, 0))
            s_fac = 1;
        if (t_west >= ot_subway_ns && t_west <= ot_subway_nesw && connects_to(t_west, 1))
            w_fac = 1;
        for (int i = 0; i < SEEX * 2; i++) {
            for (int j = 0; j < SEEY * 2; j++) {
    ...
    if (terrain_type >= ot_subway_ns && terrain_type <= ot_subway_nesw) {
        if (t_north >= ot_sewer_ns && t_north <= ot_sewer_nesw && !connects_to(terrain_type, 0)) {
            if (connects_to(t_north, 2)) {
                for (int i = SEEX - 2; i < SEEX + 2; i++) {
                    for (int j = 0; j < SEEY; j++)
                        ter(i, j) = t_sewage;
        ...
        if (t_east >= ot_sewer_ns && t_east <= ot_sewer_nesw && !connects_to(terrain_type, 1)) {
            if (connects_to(t_east, 3)) {
                for (int i = SEEX; i < SEEX * 2; i++) {
        ...
        if (t_south >= ot_sewer_ns && t_south <= ot_sewer_nesw && !connects_to(terrain_type, 2)) {
            if (connects_to(t_south, 0)) {
                for (int i = SEEX - 2; i < SEEX + 2; i++) {
        ...
        if (t_west >= ot_sewer_ns && t_west <= ot_sewer_nesw && !connects_to(terrain_type, 3)) {
            if (connects_to(t_west, 1)) {
                for (int i = 0; i < SEEX; i++) {
                    for (int j = SEEY - 2; j < SEEY + 2; j++)
                        ter(i, j) = t_sewage;
        ...
        if (t_north >= ot_subway_ns && t_north <= ot_subway_nesw && !connects_to(terrain_type, 0)) {
        ...
        if (t_east >= ot_subway_ns && t_east <= ot_subway_nesw && !connects_to(terrain_type, 1)) {
        ...
        if (t_south >= ot_subway_ns && t_south <= ot_subway_nesw && !connects_to(terrain_type, 2)) {
        ...
        if (t_west >= ot_subway_ns && t_west <= ot_subway_nesw && !connects_to(terrain_type, 3)) {
...
bool connects_to(oter_id there, int dir)
{
    ...
    case 2:
        if (there == ot_subway_ns || there == ot_subway_es || there == ot_subway_sw
            ...
            || there == ot_subway_nesw || there == ot_sewer_ns || there == ot_sewer_es
            || there == ot_sewer_sw || there == ot_sewer_nes || there == ot_sewer_nsw
            || there == ot_sewer_esw || there == ot_sewer_nesw || there == ot_ants_ns
            || there == ot_ants_es || there == ot_ants_sw || there == ot_ants_nes
            || there == ot_ants_nsw || there == ot_ants_esw || there == ot_ants_nesw)
            return true;
        return false;
    case 3:
        if (there == ot_subway_ew || there == ot_subway_sw || there == ot_subway_wn
            || there == ot_subway_new || there == ot_subway_nsw || there == ot_subway_esw
            || there == ot_subway_nesw || there == ot_sewer_ew || there == ot_sewer_sw
            || there == ot_sewer_wn || there == ot_sewer_new || there == ot_sewer_nsw
            || there == ot_sewer_esw || there == ot_sewer_nesw || there == ot_ants_ew
            || there == ot_ants_sw || there == ot_ants_wn || there == ot_ants_new
            || there == ot_ants_nsw || there == ot_ants_esw || there == ot_ants_nesw)
            return true;
        return false;
    case 0:
        if (there == ot_subway_ns || there == ot_subway_ne || there == ot_subway_wn
            ...
            || there == ot_subway_nesw || there == ot_sewer_ns || there == ot_sewer_ne
            || there == ot_sewer_wn || there == ot_sewer_nes || there == ot_sewer_new
            || there == ot_sewer_nsw || there == ot_sewer_nesw || there == ot_ants_ns
            || there == ot_ants_ne || there == ot_ants_wn || there == ot_ants_nes
            || there == ot_ants_new || there == ot_ants_nsw || there == ot_ants_nesw)
            return true;
        return false;
    case 1:
        if (there == ot_subway_ew || there == ot_subway_ne || there == ot_subway_es
            || there == ot_subway_nes || there == ot_subway_new || there == ot_subway_esw
            || there == ot_subway_nesw || there == ot_sewer_ew || there == ot_sewer_ne
            || there == ot_sewer_es || there == ot_sewer_nes || there == ot_sewer_new
            || there == ot_sewer_esw || there == ot_sewer_nesw || there == ot_ants_ew
            || there == ot_ants_ne || there == ot_ants_es || there == ot_ants_nes
            || there == ot_ants_new || there == ot_ants_esw || there == ot_ants_nesw)
            return true;
        return false;
    default:
...
void house_room(Map* m, room_type type, int x1, int y1, int x2, int y2)
{
    for (int i = x1; i <= x2; i++) {
        for (int j = y1; j <= y2; j++) {
            if (m->ter(i, j) == t_grass || m->ter(i, j) == t_dirt || m->ter(i, j) == t_floor) {
                if (j == y1 || j == y2) {
                    m->ter(i, j) = t_wall_h;
                    m->ter(i, j) = t_wall_h;
                } else if (i == x1 || i == x2) {
